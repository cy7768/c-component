import { defineComponent as k, openBlock as l, createBlock as m, unref as D, withCtx as h, renderSlot as q, mergeProps as v, ref as g, watch as G, createVNode as J, createElementBlock as c, Fragment as f, renderList as H, resolveDynamicComponent as M, createTextVNode as B, toDisplayString as C, h as T } from "vue";
...
const ru = ["onClick", "onDblclick"], nu = /* @__PURE__ */ k({
  __name: "table",
  props: {
    data: {},
    columns: { default: () => [] },
    editMode: { default: "row" },
    trigger: { default: "dblclick" }
  },
  emits: ["update:data", "save", "field-change"],
  setup(E, { expose: d, emit: s }) {
    const r = E, i = s, o = g([]), _ = g([]), S = {
      input: y,
      select: X,
      date: Z,
      number: uu,
    ...
    G(() => r.data, (u) => {
      o.value = u.map((t) => ({
        ...t,
        editing: t.editing || !1
      })), _.value.length || (_.value = JSON.parse(JSON.stringify(u)));
    }, { immediate: !0 });
    const p = g([]), b = (u, t) => p.value.some(
      (e) => e.rowIndex === u && e.prop === t
    ), w = (u, t) => {
      b(u, t) || (r.editMode === "cell" ? p.value = [{ rowIndex: u, prop: t }] : (o.value[u].editing = !o.value[u].editing, p.value.push({ rowIndex: u, prop: t })));
    }, z = (u, t) => {
      p.value = p.value.filter(
        (e) => !(e.rowIndex === u && e.prop === t)
      );
    }, L = (u, t) => u.editable ? (u.editMode || r.editMode) === "row" ? o.value[t].editing : b(t, u.prop) : !1, N = (u, t) => {
      if (r.editMode === "cell")
        return;
      o.value.forEach((a, n) => {
        a.editing && n !== u && A(n);
          ...
          default: h(() => [
            (l(!0), c(f, null, H(u.columns, (e) => (l(), m(D(K), v({
              ...
              default: h((a) => [
                e.editable ? (l(), c("div", {
                  key: 0,
                  onClick: (n) => x(a.$index, e.prop, e, "click"),
                  onDblclick: (n) => x(a.$index, e.prop, e, "dblclick")
                }, [
                  L(e, a.$index) ? (l(), m(M(V(e.type)), v({
                    key: 0,
...
const ou = (E, d) => {
  ...
  for (const [r, i] of d)
    s[r] = i;
  return s;
}, lu = /* @__PURE__ */ ou(nu, [["__scopeId", "data-v-6ef8952b"]]);
const iu = {
  CButton: eu,
  CInput: tu,
  CTable: lu
