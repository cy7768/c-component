        ...
        <el-table-column v-bind="column">
          <template #default="scope">
            <template v-if="column.editable">
              <div 
                @dblclick="handleCellDblClick(scope.$index, column.prop)"
                @click.stop="handleCellClick(scope.$index, column.prop)"
              >
                <component
                  v-if="scope.row.editing && isEditingCell(scope.$index, column.prop)"
...
const handleEdit = (index: number) => {
  ...
  originalData.value[index] = JSON.parse(JSON.stringify(props.data[index]))
}
